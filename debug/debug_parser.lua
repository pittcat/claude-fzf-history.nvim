...
local function main()
	...
	for i, q in ipairs(question_lines) do
		...
		printf("%d. 行%d: %s %s", i, q.line_num, q.content:sub(1, 60), is_ide and "[IDE命令]" or "")
	end
	print()
	...
	for i, qa in ipairs(qa_items) do
		printf("%d. 行%d-%d: %s", i, qa.buffer_line_start, qa.buffer_line_end, (qa.question or ""):sub(1, 60))
	end
	print()
	...
	printf(
		"原始问题数: %d, 解析后问题数: %d, 过滤掉: %d",
		stats.user_questions,
		#qa_items,
		stats.user_questions - #qa_items
	)
end
