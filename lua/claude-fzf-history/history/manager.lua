...
function M.get_history(opts)
	...
	logger.debug(
		"Cache status: expired=%s, items=%d, force_refresh=%s",
		tostring(cache_expired),
		#cache.items,
		tostring(opts.force_refresh)
	)
...
function M.collect_history_from_buffers()
	...
	for i, buffer_info in ipairs(claude_buffers) do
		logger.debug("Processing buffer %d/%d: %s (bufnr=%d)", i, #claude_buffers, buffer_info.name, buffer_info.bufnr)
		...
		if items then
			...
			logger.log_error_with_context("manager", "collect_history_from_buffers", "Failed to parse buffer", {
			...
			vim.notify(
				string.format("Failed to parse buffer %s: %s", buffer_info.name, err or "unknown error"),
				vim.log.levels.WARN
			)
		end
	end
...
function M.filter_items(items, opts)
	...
	for _, item in ipairs(items) do
		...
		if should_include and opts.search_term then
			...
			if not (question_lower:find(search_lower, 1, true) or answer_lower:find(search_lower, 1, true)) then
				should_include = false
...
function M.jump_to_qa(qa_item)
	...
	if line_num > buf_lines then
		...
		vim.notify(
			string.format("Line number %d is out of range (buffer has %d lines)", line_num, buf_lines),
	...
	if success then
		...
		vim.notify(string.format("Jumped to Q&A at line %d", line_num), vim.log.levels.INFO)
