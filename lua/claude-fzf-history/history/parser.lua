...
function M.detect_claude_buffer(bufnr)
	...
	logger.debug(
		"Claude buffer detection result: %s (confidence: %d)",
		is_claude_buffer and "true" or "false",
		confidence
	)
	logger.trace(
		"Detection details: name_indicator=%s, is_terminal=%s, content_score=%d",
		has_name_indicator,
		is_terminal,
		content_score
	)
...
function M.parse_claude_terminal(bufnr)
	...
	logger.debug(
		"Content analysis: score=%d, format=%s",
		content_score,
		format_detected and "detected" or "not detected"
	)
	...
	for i, line in ipairs(lines) do
		...
		if line:match("^>") then
			...
			if current_question ~= "" and current_answer ~= "" then
				...
				local should_skip = current_question:match("^/ide") or current_question:match("^%s*/ide")
				...
				if not should_skip then
					local qa_item = create_qa_item(trim(current_question), trim(current_answer), {
						id = #qa_items + 1,
		...
		elseif in_answer then
			...
			elseif
				line_stripped:match("^> /ide")
				and lines[i + 1]
				and trim(lines[i + 1]):match("^⎿%s+Connected to")
			then
			...
			elseif
				line_stripped:match("^⏺")
				or line_stripped:match("^⎿")
				or line_stripped:match("^│")
				or line_stripped:match("User approved")
				or line_stripped:match("User rejected")
			then
				if current_answer ~= "" then
					current_answer = current_answer .. "\n"
				end
...
function M.clean_response_text(text)
	...
	for _, line in ipairs(lines) do
		...
		if
			...
			or line:match("^⏺")
			or line:match("^⎿")
			or line:match("User approved")
			or line:match("User rejected")
		then
			table.insert(clean_lines, line)
			goto continue
		end
		...
		if line:match("ctrl%+r to expand") or line:match("◯") or line:match("^%s*%?%s*for shortcuts") then
			goto continue
		end
