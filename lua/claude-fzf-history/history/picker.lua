...
function M.create_history_picker(history_items, opts)
	...
	local fzf_config = vim.tbl_deep_extend("force", fzf_opts, {
		...
		header = string.format(
			"Tab: Multi-select | Enter: Jump(single only) | Ctrl-E: Export | Ctrl-F: Filter | %s: Toggle Preview | Esc: Exit",
		...
		actions = {
			["default"] = function(selected)
				...
				if #selected > 1 then
					...
					vim.notify(
						string.format(
							"Selected %d items, cannot jump.\nPlease use Ctrl-E to export or select only one item.",
							#selected
						),
						vim.log.levels.WARN,
	...
	logger.debug(
		"Preview window option: %s",
		fzf_config.fzf_opts and fzf_config.fzf_opts["--preview-window"] or "NOT SET"
	)
	logger.debug(
		"CRITICAL: Preview window config: %s",
		fzf_config.fzf_opts and fzf_config.fzf_opts["--preview-window"] or "MISSING"
	)
	...
	logger.debug(
		"Toggle preview key validation: %s -> %s",
...
function M.format_items_for_display(history_items, display_opts)
	...
	for i, item in ipairs(history_items) do
		...
		if display_opts.show_timestamp then
			timestamp = string.format("[%s] ", utils.format_timestamp(item.timestamp, display_opts.date_format))
		end
		...
		local question_text = utils.truncate_string(item.question, display_opts.max_question_length)

		local formatted_line = string.format("%s%s%s", timestamp, line_info, question_text)
...
function M.preview_qa_content(selected_line, history_items, display_opts)
	...
	if item.buffer_line_start then
		table.insert(
			...
			string.format(
				"Location: Line %d-%d",
				item.buffer_line_start,
				item.buffer_line_end or item.buffer_line_start
			)
		)
	end
...
function M.find_item_index_from_line(selected_line, history_items, display_opts)
	...
	for i, item in ipairs(history_items) do
		...
		local truncated_question = utils.truncate_string(item.question, display_opts.max_question_length)
...
function M.handle_jump_action(selected, history_items)
	...
	for i, selected_line in ipairs(selected) do
		...
		if index then
			local item = history_items[index]
			logger.debug(
				"Found item at index %d: question='%s'",
				index,
				item.question and item.question:sub(1, 50) or ""
			)
	...
	if jump_results.attempted > 1 then
		if jump_results.successful > 0 then
			vim.notify(
				string.format("Jumped to %d/%d items successfully", jump_results.successful, jump_results.attempted),
				vim.log.levels.INFO
			)
		else
...
function M.create_custom_time_filter()
	vim.ui.input({
		...
		if not start_input or start_input == "" then
			return
		end
		...
		vim.ui.input({
			...
			if not end_input or end_input == "" then
				return
			end
