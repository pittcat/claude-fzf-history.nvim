    ...
    if log_dir and log_dir ~= "" and log_dir ~= "/" then
      ...
      if not success then
        ...
        if M._config.console_logging then
          print("[claude-fzf-history] Warning: Could not create log directory, disabling file logging: " .. (err or "unknown error"))
        end
      end
    end
  end
...
function M.format_message(level, msg, ...)
  ...
  return string.format("%s[claude-fzf-history] [%s] %s%s", 
...
function M.log_function_return(module_name, func_name, result)
  M.debug("[%s] %s returned: %s", module_name, func_name, type(result) == "table" and vim.inspect(result) or tostring(result))
end
